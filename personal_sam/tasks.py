...
@shared_task()
def postulacion_email(postulante, comunicacion_pk, adjuntos):
    comunicacion = Comunicacion.objects.get(pk = comunicacion_pk)
    subject_template = Template(comunicacion.subject)
    content_template = Template(comunicacion.content)
    context = Context({"postulante":postulante})
    subject = subject_template.render(context)
    content = content_template.render(context)

    mensaje = EmailMultiAlternatives(subject = subject, body = content, from_email = "comunicaciones@montebelloacademy.org", to = ["talentohumano@montebelloacademy.org"] )
    ...
    #send_mail(u"{0}".format(subject) , u"{0}".format(content), "comunicaciones@montebelloacademy.org", ["rodrigo@montebelloacademy.org"], fail_silently = False, html_message = u"{0}".format(content))
